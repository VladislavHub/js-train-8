...
function createArray(...elements) {
  ...
  if (elements.length > 0){
    ...
    console.log("Error: No elements provided.");
  }
  return [];
}
...
function combineAndReverseArrays(arr1, arr2) {
  ...
  let duobleArray = arr1.concat(arr2);
  let reverseArray = duobleArray.reverse()
  return reverseArray
}
...
function findElementIndexes(arr, element) {
  ...
  let firtsItem = arr.indexOf(element);
  let lastItem = arr.lastIndexOf(element);

  let indexes = [];

  if(firtsItem !== 0){
    indexes.push(firtsItem);
  }

  if(lastItem !== 0 && lastItem !== firtsItem){
    indexes.push(lastItem)
  }
